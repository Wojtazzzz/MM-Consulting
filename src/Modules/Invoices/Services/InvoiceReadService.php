...
{
	public function getClientOverpayments(int $client_id): array
	{
		$order = $this->getOrder();
	...
	public function getClientUnderpayments(int $client_id): array
	{
		$order = $this->getOrder();
		$conditions = $this->getConditions($client_id);
	...
	private function getConditions(int $client_id): array
	{
		$conditions = [
			'Client.id' => $client_id,
		];
